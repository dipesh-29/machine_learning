...
class GRU_Helper():
    ...
    def train_model(self, train_data_loader):
        self.model = Sentiment_GRU(input_size=self.input_size, hidden_size=self.hidden_size, num_layers=self.num_layers, output_size=self.output_size)
...
if __name__=="__main__":
    gru_obj = GRU_Helper()
    train_data, test_data = gru_obj.load_data()
    train_loader, test_loader = gru_obj.data_loader(train_data, test_data)
    gru_obj.train_model(train_loader)
    #rnn_obj.evaluate_results(train_loader)
    gru_obj.evaluate_results(test_loader)    