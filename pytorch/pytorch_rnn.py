...
class RNN_Helper():
    ...
    def train_model(self, train_data_loader):
        self.model = Sentiment_RNN(input_size=self.input_size, hidden_size=self.hidden_size, num_layers=self.num_layers, output_size=self.output_size)
...
if __name__=="__main__":
    rnn_obj = RNN_Helper()
    train_data, test_data = rnn_obj.load_data()
    train_loader, test_loader = rnn_obj.data_loader(train_data, test_data)
    rnn_obj.train_model(train_loader)
    #rnn_obj.evaluate_results(train_loader)
    rnn_obj.evaluate_results(test_loader)    